...
class TestDataQuality:
    ...
    def test_all_pokemon_have_six_stats(self, test_db_path):
        ...
        for pokemon_id, stat_count in stat_counts:
            assert stat_count == 6, f"Pokemon {pokemon_id} has {stat_count} stats, expected 6"
    ...
    def test_species_data_has_pokemon_id_foreign_key(self, test_db_path):
        ...
        assert species_count == 20, f"Expected 20 distinct pokemon_ids in species, got {species_count}"
        ...
        assert pokemon_id_list == species_id_list, "pokemon_id foreign keys don't match pokemon_details ids"
