            ...
            <div className="flex items-center gap-1.5 text-xs text-white/40">
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M17 20h5v-2a3 3 0 00-5.354-1.853M17 20H7m10 0v-2c0-.656-.126-1.283-.354-1.853M7 20H2v-2a3 3 0 015.354-1.853M7 20v-2c0-.656.126-1.283.354-1.853m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"
                />
              </svg>
...
export default function AbilitiesPage() {
  ...
  } = useDuckDBQuery<AbilityRow>(ABILITIES_QUERY)
  const { data: effectEntries, loading: effectsLoading } =
    useDuckDBQuery<EffectEntryRow>(EFFECT_ENTRIES_QUERY)
  const { data: abilityPokemon, loading: pokemonLoading } =
    useDuckDBQuery<AbilityPokemonRow>(ABILITY_POKEMON_QUERY)
  const { data: abilityNames, loading: namesLoading } =
    useDuckDBQuery<AbilityNameRow>(ABILITY_NAMES_QUERY)
  const { data: allPokemonRaw, loading: allPokemonLoading } =
    useDuckDBQuery<PokemonRow>(POKEMON_QUERY)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm">
              Browse abilities and discover which Pokemon learn each one
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        ...
        >
          ...
          <option value="has_hidden" className="bg-[var(--surface)]">
            Has Hidden Ability
          </option>
          <option value="hidden_only" className="bg-[var(--surface)]">
            Hidden Ability Only
          </option>
        </select>
      </div>
