...
const TYPE_EFFECTIVENESS: { [attacker: string]: { [defender: string]: number } } = {
  ...
  Ground: { Fire: 2, Electric: 2, Poison: 2, Rock: 2, Ghost: 0.0, Grass: 0.5, Flying: 0.0, Steel: 2 },
...
const TYPE_COLORS: { [type: string]: string } = {
  ...
  Rock: '#B6A136', Ghost: '#705798', Dragon: '#6F35FC', Dark: '#705746', Steel: '#B7B7CE', Fairy: '#D985F2'
};

function TypeBadge({ type }: { type: string }) {
  const color = TYPE_COLORS[type] ?? '#999';
  return (
    <span className={styles.typeBadge} style={{ background: color, color: '#111' }}>{type}</span>
  );
}
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.gridToolbar}>
          ...
          <div className={styles.dropdown}>
            {availablePokemons.length > 0 ? (
              availablePokemons.map((p) => (
                <div key={p.id} className={styles.dropdownItem} onClick={() => addPokemonToFirstEmpty(p)}>
                  <span className={styles.dropdownThumb}>
                    <img src={getSpriteURL(p.id)} alt={p.name} onError={(e) => { (e.target as HTMLImageElement).src = '' }} />
                  </span>
                  <span className={styles.dropdownText}>{p.name}</span>
                  <span className={styles.dropdownTypes}>{p.type_names.map((t) => (
                    <span key={t} className={styles.smallType} style={{ background: TYPE_COLORS[t] ?? '#999' }}>{t}</span>
                  ))}</span>
                </div>
              ))
            ) : (
        ...
        <button className={styles.copyBtn} onClick={copyTeamText}>Copy Team Text</button>
      </div>
          ...
          <div key={idx} className={styles.slot} aria-label={`Slot ${idx + 1}`}>
            {p ? (
              <div className={styles.slotContent}>
                ...
                <button className={styles.removeBtn} onClick={() => removePokemonFromSlot(idx)}>Remove</button>
              </div>
            ) : (
              <div className={styles.slotEmpty} onClick={() => { /* click-to-focus search; no-op for now */ }}>
            ...
            return (
              <div key={t} className={styles.coverageCard}>
                <div className={styles.coverageHeader}>
                  ...
                  <span className={styles.badge} style={{ background: color }}>{' '} </span>
                </div>
